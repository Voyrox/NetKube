...
function initializePodTabs() {
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      panels.forEach((panel) => panel.classList.toggle("pod-tab-panel--active", panel.getAttribute("data-pod-panel") === selectedTab));
    });
  });
}
...
function renderPodDetail(item) {
  ...
  renderKeyValueList("podLabelsList", item.labels || {}, "No labels attached to this pod.");
  renderKeyValueList("podAnnotationsList", item.annotations || {}, "No annotations attached to this pod.");
}
...
function applyPodStatus(id, status) {
  ...
  if (normalized.includes("running") || normalized.includes("healthy") || normalized.includes("succeeded")) {
    element.classList.add("status-chip--healthy");
    return;
  }

  if (normalized.includes("pending") || normalized.includes("creating") || normalized.includes("init") || normalized.includes("containercreating")) {
...
function renderContainers(items) {
  setText("podContainersCount", `${items.length} container${items.length === 1 ? "" : "s"}`);
  const containerList = document.getElementById("podContainersList");
  if (!containerList) return;

  if (!items.length) {
    containerList.innerHTML = '<p class="pod-empty">No containers reported for this pod.</p>';
    return;
  }
...
function renderConditions(items) {
  ...
  if (!items.length) {
    conditionList.innerHTML = '<p class="pod-empty">No conditions reported for this pod.</p>';
    return;
  }

  conditionList.innerHTML = items.map((item) => `
    <article class="pod-collection-item">
      <div class="pod-collection-item__header">
...
function setupLogSelectors(containerNames, onChange) {
  const selectors = [document.getElementById("podLogContainerSelect"), document.getElementById("podLogsContainerSelect")];
...
async function loadPodLogs(requestQuery, container) {
  ...
  const run = async () => {
    try {
      ...
      const data = await fetchClusterData(`/api/workloads/pod/logs?${query.toString()}`);
      const content = stripAnsiSequences(data.content || "") || "No logs available for this container.";
      setText("podLogsStatus", data.container ? `Live - ${data.container}` : "Live");
      setText("podLogsStatusFull", data.container ? `Live - ${data.container}` : "Live");
...
function syncLogSelectors(container) {
  [document.getElementById("podLogContainerSelect"), document.getElementById("podLogsContainerSelect")].forEach((select) => {
    if (select && container) {
...
async function loadPodEvents(requestQuery) {
  ...
  try {
    const data = await fetchClusterData(`/api/workloads/pod/events?${requestQuery}`);
    const items = data.items || [];
    if (!items.length) {
      element.innerHTML = '<p class="pod-empty">No events reported for this pod.</p>';
      return;
    }
...
async function loadPodYAML(requestQuery) {
  try {
    const data = await fetchClusterData(`/api/workloads/pod/yaml?${requestQuery}`);
...
function renderYAMLContent(id, value) {
  ...
  element.innerHTML = lines.map((line, index) => {
    return `<div class="pod-yaml-line"><span class="pod-yaml-line-number">${index + 1}</span><span class="pod-yaml-line-content">${highlightYAML(line)}</span></div>`;
  }).join("");
}
...
function highlightYAML(line) {
  ...
  if (escaped.trimStart().startsWith("#")) {
    return `<span class="pod-yaml-comment">${escaped}</span>`;
  }
...
function highlightYAMLValue(value) {
  ...
  if (/^\d{4}-\d{2}-\d{2}t\d{2}:\d{2}:\d{2}(?:\.\d+)?z$/i.test(unquoted)) {
    return `<span class="pod-yaml-timestamp">${value}</span>`;
  }

  if (/^(true|false|null)$/i.test(trimmed)) {
    return `<span class="pod-yaml-boolean">${value}</span>`;
  }

  if (/^-?\d+(\.\d+)?$/.test(trimmed)) {
    return `<span class="pod-yaml-number">${value}</span>`;
  }
...
function renderPodError(message) {
  ...
  const ids = ["podContainersList", "podConditionsList", "podLabelsList", "podAnnotationsList", "podEventsList"];
  ids.forEach((id) => {
    const element = document.getElementById(id);
    if (element) {
