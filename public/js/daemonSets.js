...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startAutoRefresh(async () => {
    try {
      ...
      setText("daemonSetsHeroTitle", `DaemonSets (${data.stats?.healthy || 0} healthy)`);
      renderFilteredDaemonSets(searchInput?.value || "");
    } catch (error) {
      daemonSetItems = [];
...
function renderFilteredDaemonSets(query) {
  ...
  renderDaemonSetsTable(filteredItems, filteredItems.length ? "" : "No daemonsets match your search");
