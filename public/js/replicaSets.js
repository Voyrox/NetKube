...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startAutoRefresh(async () => {
    try {
      ...
      setText("replicaSetsHeroTitle", `ReplicaSets (${data.stats?.healthy || 0} healthy)`);
      renderFilteredReplicaSets(searchInput?.value || "");
    } catch (error) {
      replicaSetItems = [];
...
function renderFilteredReplicaSets(query) {
  ...
  renderReplicaSetsTable(filteredItems, filteredItems.length ? "" : "No replicasets match your search");
