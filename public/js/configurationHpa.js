...
document.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput?.addEventListener("input", () => renderFilteredHpa(searchInput.value));
...
function renderFilteredHpa(query) {
  ...
  renderHpaTable(filteredItems, filteredItems.length ? "" : "No HPA match your search");
...
function filterHpa(items, query) {
  ...
  return items.filter((item) => [item.namespace, item.name, item.target].some((value) => String(value || "").toLowerCase().includes(normalized)));
}
