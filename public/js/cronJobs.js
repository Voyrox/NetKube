...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startAutoRefresh(async () => {
    try {
      ...
      setText("cronJobsHeroTitle", `CronJobs (${data.stats?.scheduled || 0} scheduled)`);
      renderFilteredCronJobs(searchInput?.value || "");
    } catch (error) {
      cronJobItems = [];
...
function renderFilteredCronJobs(query) {
  ...
  renderCronJobsTable(filteredItems, filteredItems.length ? "" : "No cronjobs match your search");
...
function filterCronJobs(items, query) {
  ...
  return items.filter((item) => [item.namespace, item.name, item.status, item.schedule].some((value) => String(value || "").toLowerCase().includes(normalized)));
}
