...
document.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput?.addEventListener("input", () => renderFilteredResourceQuotas(searchInput.value));
  ...
  startAutoRefresh(async () => {
    try {
      ...
      setText("resourceQuotasHeroTitle", `Resource Quotas (${data.count || 0})`);
      renderFilteredResourceQuotas(searchInput?.value || "");
    } catch (error) {
      resourceQuotaItems = [];
      renderResourceQuotasTable([], error.message || "Failed to load resource quotas");
    }
  });
});
...
function renderFilteredResourceQuotas(query) {
  ...
  renderResourceQuotasTable(filteredItems, filteredItems.length ? "" : "No resource quotas match your search");
