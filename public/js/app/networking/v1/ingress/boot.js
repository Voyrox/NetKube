...
function renderFilteredIngress(query) {
  ...
  renderIngressTable(
    filteredItems,
    filteredItems.length ? "" : "No ingress match your search",
  );
...
function filterIngress(items, query) {
  ...
  return items.filter((item) =>
    [item.namespace, item.name, item.class, item.hosts, item.address].some(
      (value) =>
        String(value || "")
          .toLowerCase()
          .includes(normalized),
...
function renderIngressTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.innerHTML = `<td>${escapeHtml(item.namespace)}</td><td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.class)}</td><td>${escapeHtml(item.hosts)}</td><td>${escapeHtml(item.address)}</td><td>${escapeHtml(item.age)}</td>`;
