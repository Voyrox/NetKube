...
function renderFilteredSecrets(query) {
  ...
  renderSecretsTable(
    filteredItems,
    filteredItems.length ? "" : "No secrets match your search",
  );
...
function filterSecrets(items, query) {
  ...
  return items.filter((item) =>
    [item.namespace, item.name, item.type].some((value) =>
      String(value || "")
        .toLowerCase()
        .includes(normalized),
    ),
...
function renderSecretsTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.tabIndex = 0;
    row.innerHTML = `<td>${escapeHtml(item.namespace)}</td><td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.type)}</td><td>${escapeHtml(String(item.data))}</td><td>${escapeHtml(item.age)}</td>`;
...
function initializeSecretDrawer() {
  ...
  if (close) close.addEventListener("click", closeSecretDrawer);
  if (backdrop) backdrop.addEventListener("click", closeSecretDrawer);
}
async function openSecretData(item) {
  ...
  try {
    ...
    renderYamlDrawerContent(
      "secretDataContent",
      data.content || "No secret data available.",
    );
  } catch (error) {
    setDrawerText(
      "secretDataContent",
      error.message || "Failed to load secret data.",
    );
  }
}
