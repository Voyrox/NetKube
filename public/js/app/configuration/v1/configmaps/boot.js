...
function renderFilteredConfigMaps(query) {
  ...
  renderConfigMapsTable(
    filteredItems,
    filteredItems.length ? "" : "No configmaps match your search",
  );
...
function filterConfigMaps(items, query) {
  ...
  return items.filter((item) =>
    [item.namespace, item.name, item.immutable].some((value) =>
      String(value || "")
        .toLowerCase()
        .includes(normalized),
    ),
...
function renderConfigMapsTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.innerHTML = `<td>${escapeHtml(item.namespace)}</td><td>${escapeHtml(item.name)}</td><td>${escapeHtml(String(item.data))}</td><td>${escapeHtml(item.immutable)}</td><td>${escapeHtml(item.age)}</td>`;
