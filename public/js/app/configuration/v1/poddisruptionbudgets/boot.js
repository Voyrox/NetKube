  ...
  startAutoRefresh(async () => {
    try {
      const data = await fetchClusterData(
        "/api/configuration/poddisruptionbudgets",
      );
      pdbItems = data.items || [];
      ...
      renderPdbTable(
        [],
        error.message || "Failed to load pod disruption budgets",
      );
    }
  });
});
function renderFilteredPdb(query) {
  ...
  renderPdbTable(
    filteredItems,
    filteredItems.length ? "" : "No pod disruption budgets match your search",
  );
