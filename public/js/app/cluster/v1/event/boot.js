  ...
  startAutoRefresh(async () => {
    try {
      const data = await fetchClusterData(
        `/api/cluster/event${query.toString() ? `?${query.toString()}` : ""}`,
      );
...
function getEventHeroTitle(item) {
  return (
    item.name || item.involvedObject || item.kind || item.reason || "Event"
  );
}
function getEventHeadline(item) {
  if (item.reason && item.kind && item.name)
    return `${item.reason} - ${item.kind}/${item.name}`;
  if (item.reason && item.name) return `${item.reason} - ${item.name}`;
...
function renderEventTimeline(items) {
  ...
  if (!items.length) {
    element.innerHTML =
      '<p class="cluster-empty">No related event history found.</p>';
    return;
  }
