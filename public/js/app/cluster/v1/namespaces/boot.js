...
function renderNamespacesTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.tabIndex = 0;
    row.innerHTML = `<td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.phase)}</td><td>${escapeHtml(item.age)}</td>`;
...
function initializeNamespaceDrawer() {
  ...
  if (close) close.addEventListener("click", closeNamespaceDrawer);
  if (backdrop) backdrop.addEventListener("click", closeNamespaceDrawer);
}
async function openNamespaceYaml(item) {
  ...
  try {
    ...
    setDrawerText(
      "namespaceYamlContent",
      error.message || "Failed to load namespace YAML.",
    );
  }
}
