...
function renderNodeLabels(labels) {
  ...
  if (!entries.length) {
    element.innerHTML =
      '<li><span class="cluster-tag-key">No labels</span><strong class="cluster-tag-value">-</strong></li>';
    return;
  }
  element.innerHTML = entries
    .map(
      ([key, value]) =>
        `<li><span class="cluster-tag-key">${escapeHtml(key)}</span><strong class="cluster-tag-value">${escapeHtml(value || "-")}</strong></li>`,
    )
    .join("");
}
function renderNodeConditions(items) {
  ...
  if (!items.length) {
    element.innerHTML =
      '<p class="cluster-empty">No conditions reported for this node.</p>';
    return;
  }
  element.innerHTML = items
    .map(
      (item) =>
        `<article class="cluster-timeline-item"><div class="cluster-timeline-top"><span class="cluster-timeline-title">${escapeHtml(item.type || "Condition")}</span><span class="cluster-event-badge ${badgeClass(item.status)}">${escapeHtml(item.status || "Unknown")}</span></div><p class="cluster-timeline-message">${escapeHtml(item.reason || "-")}${item.message && item.message !== "-" ? ` - ${escapeHtml(item.message)}` : ""}</p></article>`,
    )
    .join("");
}
function renderNodeTimeline(items) {
  ...
  if (!items.length) {
    element.innerHTML =
      '<p class="cluster-empty">No recent node activity found.</p>';
    return;
  }
  element.innerHTML = items
    .map(
      (item) =>
        `<article class="cluster-timeline-item"><div class="cluster-timeline-top"><span class="cluster-timeline-title">${escapeHtml(item.title || "Event")}</span><span class="cluster-timeline-time">${escapeHtml(item.age || "-")}</span></div><p class="cluster-timeline-message">${escapeHtml(item.message || "-")}</p></article>`,
    )
    .join("");
}
function badgeClass(status) {
  const normalized = String(status || "").toLowerCase();
  if (normalized === "true" || normalized.includes("ready"))
    return "cluster-event-badge--healthy";
