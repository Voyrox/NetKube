...
function renderLeasesTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.tabIndex = 0;
    row.innerHTML = `<td>${escapeHtml(item.namespace)}</td><td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.holder)}</td><td>${escapeHtml(item.lastRenew)}</td><td>${escapeHtml(item.age)}</td>`;
...
function initializeLeaseDrawer() {
  ...
  if (close) close.addEventListener("click", closeLeaseDrawer);
  if (backdrop) backdrop.addEventListener("click", closeLeaseDrawer);
}
async function openLeaseYaml(item) {
  ...
  try {
    ...
    setDrawerText(
      "leaseYamlContent",
      error.message || "Failed to load lease YAML.",
    );
  }
}
