...
function highlightYamlPunctuation(value) {
  return value.replace(
    /[{}\[\]]/g,
    (char) => `<span class="nk-yaml-punctuation">${char}</span>`,
  );
}
...
function highlightYamlLine(line) {
  ...
  return `${highlightYamlPunctuation(nbsp(prefix))}<span class="nk-yaml-key">${highlightYamlPunctuation(nbsp(key))}</span>:${value ? ` ${highlightYamlValue(highlightYamlPunctuation(nbsp(value)))}` : ""}`;
}
...
function renderHighlightedYaml(value) {
  return String(value || "")
    ...
    .map(
      (line, index) =>
        `<div class="nk-yaml-line"><span class="nk-yaml-line-number">${index + 1}</span><span class="nk-yaml-line-content">${highlightYamlLine(line)}</span></div>`,
    )
    .join("");
}
...
function renderHighlightedYamlContent(value) {
  return String(value || "")
    ...
    .map(
      (line) =>
        `<div class="nk-yaml-editor-line">${highlightYamlLine(line)}</div>`,
    )
    .join("");
}
