document.addEventListener("DOMContentLoaded", () => {
  startAutoRefresh(async () => {
    try {
      ...
      renderWorkloadMetric("pods", data.pods, [
        "Running",
        "Pending",
        "Failed",
        "Other",
      ]);
      renderWorkloadMetric(
        "deployments",
        data.deployments,
        ["Healthy", "Warning", "Pending", "Other"],
        { hideStatus: true },
      );
      renderWorkloadMetric("replicaSets", data.replicaSets, [
        "Ready",
        "Pending",
        "Issues",
        "History",
      ]);
      renderWorkloadMetric("daemonSets", data.daemonSets, [
        "Ready",
        "Pending",
        "Issues",
        "Other",
      ]);
      renderWorkloadMetric("statefulSets", data.statefulSets, [
        "Ready",
        "Updating",
        "Issues",
        "Other",
      ]);
      renderWorkloadMetric("cronJobs", data.cronJobs, [
        "Tracked",
        "Warning",
        "Issues",
        "Other",
      ]);
      renderWorkloadMetric("jobs", data.jobs, [
        "Succeeded",
        "Active",
        "Failed",
        "Other",
      ]);
      renderWorkloadMetric("resourceQuotas", data.resourceQuotas, [
        "Tracked",
        "Warning",
        "Issues",
        "Other",
      ...
      renderWarnings(
        "workloadWarningsList",
        "workloadWarningsEmpty",
        data.warnings,
      );
    } catch (error) {
      setText(
        "workloadWarningsEmpty",
        error.message || "Failed to load workloads overview",
      );
    }
  });
});
...
function renderWorkloadMetric(prefix, metric, labels, options = {}) {
  ...
  const other = Math.max(
    Number(
      metric.other != null ? metric.other : total - primary - warning - danger,
    ),
    0,
  );
  const primaryOnly =
  ...
  updateSummaryHealthBar(prefix, total, {
    primary,
    warning,
    danger,
    other,
    healthTotal: Number(metric.healthTotal || 0),
  });
