...
document.addEventListener("DOMContentLoaded", () => {
  ...
  startAutoRefresh(async () => {
    try {
      ...
      setText(
        "statefulSetsHeroTitle",
        `StatefulSets (${data.stats?.healthy || 0} healthy)`,
      );
      renderFilteredStatefulSets(searchInput?.value || "");
    } catch (error) {
      statefulSetItems = [];
      renderStatefulSetsTable(
        [],
        error.message || "Failed to load statefulsets",
      );
    }
  });
});
...
function renderFilteredStatefulSets(query) {
  ...
  renderStatefulSetsTable(
    filteredItems,
    filteredItems.length ? "" : "No statefulsets match your search",
  );
