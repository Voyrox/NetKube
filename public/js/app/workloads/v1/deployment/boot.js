  ...
  startAutoRefresh(async () => {
    try {
      const data = await fetchClusterData(
        `/api/workloads/deployment?${requestQuery}`,
      );
      ...
      renderDeploymentError(
        error.message || "Failed to load deployment details",
      );
    }
  });
});
    ...
    tab.addEventListener("click", () => {
      ...
      panels.forEach((panel) =>
        panel.classList.toggle(
          ...
          panel.getAttribute("data-deployment-panel") === selected,
        ),
      );
    });
  });
...
function renderDeploymentDetail(item) {
  ...
  renderTagList(
    "deploymentLabelsList",
    item.labels || {},
    "No labels attached to this deployment.",
  );
  renderAnnotationList(
    "deploymentAnnotationsList",
    item.annotations || {},
    "No annotations attached to this deployment.",
  );
}
...
function renderDeploymentConditions(items) {
  ...
  if (!items.length) {
    list.innerHTML =
      '<p class="pod-empty">No conditions reported for this deployment.</p>';
    return;
  }
...
function renderReplicaSets(items) {
  ...
  if (!items.length) {
    list.innerHTML =
      '<p class="pod-empty">No replica sets matched this deployment.</p>';
    return;
  }
  list.innerHTML = items
    .map(
...
function renderPods(items) {
  ...
  if (!items.length) {
    list.innerHTML =
      '<p class="pod-empty">No pods matched this deployment.</p>';
    return;
  }
  list.innerHTML = items
    .map(
...
async function loadDeploymentEvents(requestQuery) {
  ...
  try {
    const data = await fetchClusterData(
      `/api/workloads/deployment/events?${requestQuery}`,
    );
    const items = data.items || [];
    if (!items.length) {
      list.innerHTML =
        '<p class="pod-empty">No events reported for this deployment.</p>';
      return;
    }
    list.innerHTML = items
      .map(
...
async function loadDeploymentYAML(requestQuery) {
  try {
    const data = await fetchClusterData(
      `/api/workloads/deployment/yaml?${requestQuery}`,
    );
    renderYAMLContent(
      "deploymentYamlContent",
      data.content || "No YAML available.",
    );
  } catch (error) {
    renderYAMLContent(
      "deploymentYamlContent",
      error.message || "Failed to load YAML",
    );
  }
}
...
function renderDeploymentError(message) {
  ...
  [
    "deploymentConditionsList",
    "deploymentReplicaSetsList",
    "deploymentPodsList",
    "deploymentEventsList",
    "deploymentLabelsList",
    "deploymentAnnotationsList",
  ].forEach((id) => {
    const element = document.getElementById(id);
