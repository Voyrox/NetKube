...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (heroUser)
      heroUser.textContent =
        context.userName || context.contextName || "Unknown user";
    if (selectedContext)
      selectedContext.textContent =
        context.clusterName || context.contextName || "Unknown cluster";
    if (lastRefresh)
...
function applyOverviewFallback(heroUser, selectedContext, lastRefresh) {
  if (heroUser) heroUser.textContent = "No cluster selected";
  if (selectedContext)
    selectedContext.textContent = "Choose a cluster from home";
  if (lastRefresh) lastRefresh.textContent = "not connected";
}
