...
function renderServicesTable(items, message) {
  ...
  items.forEach((item) => {
    ...
    row.innerHTML = `
      ...
      <td>${formatServicePorts(item.ports)}</td>
...
function formatServicePortEntry(entry) {
  const match = entry.match(/^([A-Z]+)\/(\S+?)(?:\s*->\s*(\S+))?(?:\s*\(([^)]+)\))?$/);
  if (!match) {
  ...
  return `
    <div class="service-port-entry">
      <span class="service-port-entry__value">${escapeHtml(`${port}:${target}`)}</span>
      <span class="service-port-entry__meta">/${escapeHtml(protocol)}${escapeHtml(label)}</span>
    </div>
  `;
}
