...
func DetailHandler(c *gin.Context) {
	...
	item, err := DetailFor(cluster.Clientset, strings.TrimSpace(c.Query("name")))
	if err != nil {
		c.JSON(http.StatusBadGateway, shared.APIError{Error: err.Error()})
		return
	}
	c.JSON(http.StatusOK, DetailResponse{Meta: shared.PageMetaFromCluster(cluster, ""), Item: item})
}
...
func YAMLHandler(c *gin.Context) {
	...
	if name == "" {
		c.JSON(http.StatusBadRequest, shared.APIError{Error: "node name is required"})
		return
	}
